...
async def evaluate_single_log(log_path: Path, log_meta: dict) -> dict:
    ...
    try:
        ...
        result = {
            ...
            'match_severity': log_meta['severity_level_id'] == prediction['severity_level_id'],
        ...
        is_token_limit = any(keyword in error_msg for keyword in [
            'token', 'limit', 'quota', 'exceeded', 'токен', 'лимит', 'превышен'
        ])
...
async def evaluate_test_set(test_logs_dir: str = "test_logs", output_file: str = "evaluation_results.json"):
        ...
        if result['success']:
            ...
            if not result['match_severity'] or not result['match_threat']:
                ...
                if not result['match_severity']:
                    details.append(f"Severity: ожидалось {result['actual_severity']}, получено {result['predicted_severity']}")
                if not result['match_threat']:
                    details.append(f"Threat: ожидалось {result['actual_threat']}, получено {result['predicted_threat']}")
            ...
            if result.get('token_limit', False):
                ...
                print()
                print("⚠️" * 40)
                print(f"   Исчерпан лимит токенов GigaChat API!")
                print(f"   Успешно обработано: {successful}/{i} логов")
                print(f"   Оценка будет произведена на доступных данных.")
                print("⚠️" * 40)
                print()
    ...
    if token_limit_reached:
        print(f"⚠️  Анализ прерван из-за лимита токенов!")
    ...
    if successful > 0:
        ...
        for level, name in severity_names.items():
            ...
            if m['support'] > 0:
                print(f"   {level}. {name:12s} - P: {m['precision']:.2%}  R: {m['recall']:.2%}  F1: {m['f1_score']:.2%}  (n={m['support']})")
        print()
        
        # Threat type metrics
        print("🎯 Метрики по типам угроз (Threat Type):")
        threat_names = labels_data['threat_types']
        for threat_id, threat_info in sorted(threat_names.items(), key=lambda x: int(x[0])):
            threat_id = int(threat_id)
            m = metrics['threat_metrics'][threat_id]
            if m['support'] > 0:
                print(f"   {threat_id:2d}. {threat_info['name']:15s} - P: {m['precision']:.2%}  R: {m['recall']:.2%}  F1: {m['f1_score']:.2%}  (n={m['support']})")
        print()
        
        # Средние метрики
        avg_severity_f1 = sum(m['f1_score'] for m in metrics['severity_metrics'].values() if m['support'] > 0) / 4
        avg_threat_f1 = sum(m['f1_score'] for m in metrics['threat_metrics'].values() if m['support'] > 0) / 11
