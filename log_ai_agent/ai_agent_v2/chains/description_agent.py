...
ПРАВИЛА ВЫБОРА KEYWORDS (ЕСЛИ есть соответствующие признаки в логах):
  * PowerShell команды → добавь ["PowerShell", "выполнение команд", "скрипты", "обфускация"]
  * sc.exe / создание служб → добавь ["sc.exe", "создание служб", "персистенция", "T1543"]
  * schtasks / планировщик → добавь ["schtasks", "планировщик задач", "T1053"]
  * rundll32 / MiniDump / LSASS → добавь ["rundll32", "MiniDump", "LSASS", "кража учетных данных", "T1003"]
  * ld.so / LD_PRELOAD → добавь ["ld.so", "preload", "инжекция", "T1574"]
  * Множественные неудачные логины (более 5 с одного IP) → добавь ["брутфорс", "подбор паролей", "T1110"]
  * HTTP методы DELETE, PUT, PATCH → НЕ добавляй keywords для RAG-поиска (это шум, не атака)
  * HTTP 5xx ошибки (500, 502, 503, 504) → НЕ добавляй keywords (это шум, не атака)
  * Легитимные команды (sudo, systemctl, docker, kubectl) → НЕ добавляй keywords без других признаков атаки

ВАЖНО: 
- Добавляй ТОЛЬКО те keywords, которые соответствуют реальным командам в логах
- Если в логах нет PowerShell — НЕ добавляй keywords про PowerShell
