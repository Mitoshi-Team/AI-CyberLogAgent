...
class PrimaryAnalysisResult(BaseModel):
    ...
    events_found: int = Field(default=0, description="Number of suspicious events found")
...
class MITREResult(BaseModel):
    ...
    techniques: list[dict] = Field(default_factory=list, description="List of matched MITRE techniques")
...
class FullPipelineResult(BaseModel):
    ...
    primary_analysis: Optional[PrimaryAnalysisResult] = Field(None, description="Agent 1 result")
    mitre_result: Optional[MITREResult] = Field(None, description="RAG result")
    final_result: Optional[FinalAnalysisResult] = Field(None, description="Agent 2 result")
