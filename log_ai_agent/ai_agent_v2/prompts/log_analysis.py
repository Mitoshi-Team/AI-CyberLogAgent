...
ЗАДАЧА:
1. Сопоставь КАЖДУЮ группу подозрительной активности с техниками MITRE ATT&CK
2. Оцени уровень серьёзности для КАЖДОЙ техники (severity_level_id: 1-4)
3. Определи итоговый (максимальный) уровень серьёзности
4. Сформируй подробный отчёт со списком всех найденных инцидентов
5. Дай практические рекомендации по устранению

ВАЖНО: Один отчёт может содержать НЕСКОЛЬКО инцидентов (несколько техник MITRE).

ФОРМАТ ОТВЕТА:
## Отчёт об инциденте безопасности

### Описание инцидентов
Подробное описание всех найденных активностей.
...
### Уровень серьёзности
[Максимальная оценка и обоснование]
...
ЗАДАЧА:
1. Опиши инциденты, объединяя все источники
2. Покажи как YARA/Sigma совпадения подтверждают выводы AI
3. Для КАЖДОГО инцидента укажи технику MITRE и тактику
4. Дай рекомендации по устранению и предотвращению
5. Сохрани/обоснуй оценку серьёзности для каждого инцидента
6. НЕПОДТВЕРЖДЕННЫЕ события вынеси в отдельный блок (не влияют на overall_severity!)

ФОРМАТ ОТВЕТА:
## Отчёт об инцидентах безопасности
